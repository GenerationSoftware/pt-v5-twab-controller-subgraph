...
export function createNewTotalSupplyTwabEvent(
    ...
    newTotalSupplyTwabEvent.parameters.push(
        new ethereum.EventParam('newTotalSupplyTwab', ethereum.Value.fromTuple(newTotalSupplyTwab)),
    );

    return newTotalSupplyTwabEvent;
}

export function createIncreasedBalanceEvent(
    vault: Address,
    user: Address,
    newTwab: ethereum.Tuple,
): IncreasedBalance {
    let newUserTwabEvent = changetype<IncreasedBalance>(newMockEvent());
